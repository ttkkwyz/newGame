...
export class PauseDialog extends Phaser.GameObjects.Container {
    ...
    constructor(
        ...
        const titleText = scene.add.text(0, -140, 'PAUSE', { fontSize: '28px', fontStyle: 'bold' }).setOrigin(0.5);
        this.add([menuBg, titleText]);

        const menuItems = [
            { text: 'ゲームを続ける', y: -50, action: callbacks.onResume },
            { text: 'ルール確認', y: 20, action: callbacks.onRules },
            { text: 'トップに戻る', y: 90, action: callbacks.onTop }
        ];
